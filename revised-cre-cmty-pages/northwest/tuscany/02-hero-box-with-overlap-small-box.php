<div class="about-hero-section section-spacer" id="more">
    <div class="about-hero-section--1 p-6" style="background: rgb(2, 0, 36); background: linear-gradient(to bottom, rgba(2, 0, 36, 0.9) 0%, rgba(2, 0, 36, 0.713203323321516) 100%), url('https://www.calgary-real-estate.com/uploads/agent-1/community/nw-calgary/tuscany/amenities/tuscany-wm-12.webp'); background-position: center bottom;">
        <div>
            <h2>Tuscany <span class="text-with-line">Real Estate</span></h2>
            <p>Tuscany homes can boast of being in one of the few best selling communities in the North West neighborhood of Calgary real estate. </p>
        </div>
    </div>
    <div class="about-hero-section--2 p-6">
        <h3>Explore Tuscany</h3>
        <ul class="list-with-icons">
            <li>
                <img src="https://www.calgary-real-estate.com/uploads/agent-1/icons/checked.png" alt="checkmark icon" width="32" height="32">
                <a href="#about-the-community" class="p-2  d-inline-block">About the Community</a>
            </li>
            <li>
                <img src="https://www.calgary-real-estate.com/uploads/agent-1/icons/checked.png" alt="checkmark icon" width="32" height="32">
                <a href="#community-vibe" class="p-2  d-inline-block">Community Vibe</a>
            </li>
            <li>
                <img src="https://www.calgary-real-estate.com/uploads/agent-1/icons/checked.png" alt="checkmark icon" width="32" height="32">
                <a href="#agent-guided-video-tour" class="p-2  d-inline-block">Agent-Guided Video Tour</a>
            </li>
            <li>
                <img src="https://www.calgary-real-estate.com/uploads/agent-1/icons/checked.png" alt="checkmark icon" width="32" height="32">
                <a href="#schools" class="p-2  d-inline-block">Schools In or Near Tuscany</a>
            </li>
        </ul>
    </div>
</div>